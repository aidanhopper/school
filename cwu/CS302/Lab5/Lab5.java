...
public class Lab5
{
	...
	if (occs > 1) {
	  ...
	  int conTracker = 0;
	  // create map to check if pair aleady exists
	  ...
	  for (int k = 0; k < numOfArrs; k++) {
	    ...
	    String s1 = String.format("%d:%d", pair[0], pair[1]);
	    String s2 = String.format("%d:%d", pair[1], pair[0]);
	    if (pair[0] == null) { 
		pair[0] = i;
		conTracker++;
	    }
	    else if (pair[1] == null) { 
	      if (!pairs.getOrDefault(s2, false)) {
		pair[1] = i;
		conTracker++;
		pairs.put(s1, true);
		pairs.put(s2, true);
      ...
      // looks for pair that is less than or equal to m
      for (int i = 0; i < arr.length; i++) {
	int occs = occ.get(arr[i]); 
	if (occs > 1) {
	  int numOfArrs = (occs * (occs - 1)) / 2;
